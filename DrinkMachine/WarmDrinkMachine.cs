    ...
    public class WarmDrinkMachine
    {
        ...
        public WarmDrinkMachine()
        {
            foreach (var t in typeof(WarmDrinkMachine).Assembly.GetTypes())
            {
                ...
                {
                    namedFactories.Add(Tuple.Create(t.Name.Replace("Factory", string.Empty),
        ...
        public IWarmDrink MakeDrink()
        {
            ...
            {
                string s;
                if ((s = Console.ReadLine()) != null && int.TryParse(s, out int i) && i >= 0 && i < namedFactories.Count)
                {
                    Console.Write("How much: ");
                    s = Console.ReadLine();
                    if (s != null && int.TryParse(s, out int total) && total > 0)
                    {
